        ...
        for key, val in context.items():
            ...
            if header == "lighting":
                ...
                if attr == "pos_x":
                    ...
                    raise ValueError(f"Wrong attribute for the lighting modder: '{attr}'")
    ...
    def save_defaults(self):
        ...
        for name in self.geom_names:
            if self._check_geom_for_texture(name):
                ...
                self._defaults[name]["material_reflectance"] = self.get_material_reflectance(name)
                self._defaults[name]["material_shininess"] = self.get_material_shininess(name)
                self._defaults[name]["material_specular"] = self.get_material_specular(name)
            else:
                # store geom color
    ...
    def get_context(self):
        ...
        for name in self.geom_names:
            ...
            if self._check_geom_for_texture(name):
                ...
                context[f"{header}@{name}@material_reflectance"] = np.array(self.get_material_reflectance(name))
                context[f"{header}@{name}@material_shininess"] = np.array(self.get_material_shininess(name))
                context[f"{header}@{name}@material_specular"] = np.array(self.get_material_specular(name))
...
class DynamicsModder(BaseModder):
    ...
    def save_defaults(self):
        ...
        for geom_name in self.sim.model.geom_names:
            ...
            self.geom_defaults[geom_name] = {
                ...
                "friction_torsional": np.array(self.sim.model.geom_friction[geom_id][1]),
    ...
    def get_context(self):
        ...
        for body_name in self.sim.model.body_names:
            body_id = self.sim.model.body_name2id(body_name)
            context[f"{header}@{body_name}@position"] = np.array(self.sim.model.body_pos[body_id])
            context[f"{header}@{body_name}@quaternion"] = np.array(self.sim.model.body_quat[body_id])
            # print (context[f"{header}@{body_name}@quaternion"],"check")
            context[f"{header}@{body_name}@euler"] = trans.quat2euler(self.sim.model.body_quat[body_id])
            # context[f"{header}@{body_name}@inertia"] = np.array(self.sim.model.body_inertia[body_id])
            context[f"{header}@{body_name}@inertia_ixx"] = self.sim.model.body_inertia[body_id][0]
            context[f"{header}@{body_name}@inertia_iyy"] = self.sim.model.body_inertia[body_id][1]
            context[f"{header}@{body_name}@inertia_izz"] = self.sim.model.body_inertia[body_id][2]
            context[f"{header}@{body_name}@mass"] = self.sim.model.body_mass[body_id]

        for geom_name in self.sim.model.geom_names:
            geom_id = self.sim.model.geom_name2id(geom_name)
            context[f"{header}@{geom_name}@friction_sliding"] = np.array(self.sim.model.geom_friction[geom_id][0])
            context[f"{header}@{geom_name}@friction_torsional"] = np.array(self.sim.model.geom_friction[geom_id][1])
            context[f"{header}@{geom_name}@friction_rolling"] = np.array(self.sim.model.geom_friction[geom_id][2])
            context[f"{header}@{geom_name}@time_constant"] = np.array(self.sim.model.geom_solref[geom_id][0])
            context[f"{header}@{geom_name}@damping_ratio"] = np.array(self.sim.model.geom_solref[geom_id][1])
            # context[f"{header}@{geom_name}@solref"] = np.array(self.sim.model.geom_solref[geom_id])
            context[f"{header}@{geom_name}@solimp"] = np.array(self.sim.model.geom_solimp[geom_id])

        for joint_name in self.sim.model.joint_names:
            joint_id = self.sim.model.joint_name2id(joint_name)
            dof_idx = [i for i, v in enumerate(self.sim.model.dof_jntid) if v == joint_id]
            context[f"{header}@{joint_name}@stiffness"] = self.sim.model.jnt_stiffness[joint_id]
            context[f"{header}@{joint_name}@frictionloss"] = np.array(self.sim.model.dof_frictionloss[dof_idx])
            context[f"{header}@{joint_name}@damping"] = np.array(self.sim.model.dof_damping[dof_idx])
            context[f"{header}@{joint_name}@armature"] = np.array(self.sim.model.dof_armature[dof_idx])
